    ...
    with path.open("r", encoding="utf-8") as handle:
        for idx, line in enumerate(handle, start=1):
            ...
            if previous_ts is not None:
                metrics.duration_ns = max(frame.timestamp_ns - previous_ts, metrics.duration_ns)
...
def build_validation_tasks(metrics: ReplayMetrics, target_hz: int) -> List[ReplayTask]:
    ...
    add(
        "jsonl_schema_valid",
        "pass" if metrics.frames_invalid == 0 else "fail",
        f"{metrics.frames_valid} valid frames, {metrics.frames_invalid} invalid frames",
    )
    add(
        "tick_ordering",
        "pass" if metrics.max_tick_gap <= 1 else "warn",
...
def replay_artifact(request: ReplayRequest) -> ReplayResponse:
    ...
    tasks = build_validation_tasks(metrics, request.sampling_hz)
