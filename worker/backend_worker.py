...
def process_fix_ci_job(job: dict) -> None:
    ...
    try:
        ...
        # FIXED: 5 arguments (job_id, trace_id, span_name, status, attributes)
        ...
        set_job_phase(job_id, "running", "token_issued", {}, None)
        ...
        try:
            ...
            set_job_phase(job_id, "running", "cloned", {}, None)
            ...
            add_span(job_id, trace_id, "apply_patch", "ok", {"hash": hashlib.sha256(patch.encode()).hexdigest()})
            set_job_phase(job_id, "running", "patched", {}, None)
            ...
            if not tests_ok:
                set_job_phase(job_id, "failed", "validation_failed", {"tests_ok": False}, "Tests failed")
                return

            set_job_phase(job_id, "running", "validated", {"tests_ok": True}, None)
            ...
            # FIXED: 4 arguments (job_id, fix_branch, pr_url, result_payload)
            complete_job(job_id, fix_branch, pr_url, {"summary": "PR opened"})
        ...
        # FIXED: 5 arguments (job_id, status, phase, payload, error_message)
        set_job_phase(job_id, "failed", "error", {"exception": str(e)}, str(e))
