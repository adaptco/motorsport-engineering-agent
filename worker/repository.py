...
def set_job_phase(job_id: str, status: str, phase: str, payload: dict | None = None, error_message: str | None = None):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            ''',
            (status, phase, json.dumps(payload) if payload is not None else None, error_message, job_id)
        )
        cur.execute(
            "INSERT INTO job_events (job_id, level, event_type, payload) VALUES (%s, %s, %s, %s::jsonb)",
            (job_id, "ERROR" if error_message else "INFO", f"job.{phase}", json.dumps(payload or {"error": error_message} if error_message else {}))
        )
...
def get_job_identity(job_id: str):
    with get_conn() as conn, conn.cursor() as cur:
        ...
        row = cur.fetchone()
        return {"trace_id": str(row[0]), "repo_slug": row[1], "base_branch": row[2]} if row else None
...
def complete_job(job_id: str, fix_branch: str, pr_url: str, result_payload: dict):
    with get_conn() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            ...
            (job_id, json.dumps({"source": "backend_worker"}), json.dumps({"status": "complete"}), json.dumps(result_payload))
        )
