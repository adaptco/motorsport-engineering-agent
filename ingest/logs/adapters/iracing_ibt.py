...
def parse_iracing_ibt(path: Path) -> ParsedLog:
    try:
        ...
        raise RuntimeError("libibt is required to parse iRacing .ibt files. Install with: pip install libibt") from exc
    ...
    if laps is not None and getattr(laps, "num_rows", 0) > 0:
        ...
        if {"num", "start_time", "end_time"}.issubset(lap_frame.columns) and "timestamp_ms" in frame.columns:
            frame["lap_index"] = 0
            for _, lap in lap_frame.iterrows():
                mask = (frame["timestamp_ms"] >= lap["start_time"]) & (frame["timestamp_ms"] < lap["end_time"])
                frame.loc[mask, "lap_index"] = int(lap["num"])
            notes.append("lap_index derived from libibt lap intervals")
    return ParsedLog(vendor="iracing", source_path=path, frame=frame, metadata=metadata, notes=notes)
