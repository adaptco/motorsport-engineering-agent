...
def test_startup_fails_when_webhook_required_without_secret(monkeypatch):
    import importlib

    monkeypatch.setenv("GITHUB_WEBHOOK_REQUIRED", "true")
    monkeypatch.delenv("GITHUB_WEBHOOK_SECRET", raising=False)

    app_module = importlib.import_module("control_plane.app")
    importlib.reload(app_module)

    try:
        with TestClient(app_module.app):
            raise AssertionError("startup should fail without webhook secret")
    except RuntimeError as exc:
        assert "GITHUB_WEBHOOK_SECRET must be set" in str(exc)
    finally:
        monkeypatch.delenv("GITHUB_WEBHOOK_REQUIRED", raising=False)
        importlib.reload(app_module)
