...
def test_webhook_valid_signature_is_accepted(monkeypatch):
    ...
    monkeypatch.setattr(
        "control_plane.webhooks.correlate_workflow_run", lambda *args, **kwargs: None
    )
...
def test_startup_validation_returns_configured_state():
    assert (
        validate_webhook_startup_config(webhook_secret="test-secret", webhook_required=False)
        is True
    )
