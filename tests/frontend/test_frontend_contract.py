...
def test_frontend_shell_and_modules_exist():
    required = [
        ...
        "lib/events.js",
        "lib/utils.js",
        "pages/mission-control.js",
        "pages/aero-studio.js",
        "pages/race-engineering.js",
        "pages/operations.js",
        "pages/not-found.js",
    ]
    assert all((FRONTEND / path).is_file() for path in required)
...
def test_shell_uses_static_app_and_four_routes():
    ...
    for route in ("mission-control", "aero-studio", "race-engineering", "operations"):
        assert f'data-route="{route}"' in html


def test_api_client_enforces_request_id_and_deduplicates_polls():
    js = (FRONTEND / "lib/api.js").read_text()
...
def test_runtime_state_endpoints_are_mounted_by_included_session_router():
    backend = (ROOT / "control_plane/routes/session.py").read_text()
    assert '@router.get("/runtime-state/snapshot"' in backend
    assert '@router.get("/runtime-state/events"' in backend
    assert '@router.post("/runtime-state/events"' in backend


def test_frontend_endpoint_references_match_contract():
    refs = set()
    for path in (FRONTEND / "pages").glob("*.js"):
        refs.update(
            re.findall(
