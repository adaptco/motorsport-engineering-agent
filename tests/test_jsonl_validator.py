...
def test_jsonl_validator_monotonicity_violations(tmp_path: Path) -> None:
    ...
    rows = [
        {"session_id": "s1", "driver_id": "d1", "track_id": "t1", "car_id": "c1", "timestamp_ns": 500, "tick": 5, "channels": {"Throttle": 0.5}},
        {"session_id": "s1", "driver_id": "d1", "track_id": "t1", "car_id": "c1", "timestamp_ns": 400, "tick": 5, "channels": {"Throttle": 0.6}},
    ]
    path.write_text("".join(json.dumps(r) + "\n" for r in rows), encoding="utf-8")
...
def test_jsonl_validator_all_valid(tmp_path: Path) -> None:
    ...
    rows = [
        {"session_id": "s1", "driver_id": "d1", "track_id": "t1", "car_id": "c1", "timestamp_ns": 100, "tick": 1, "channels": {"Throttle": 0.0}},
        {"session_id": "s1", "driver_id": "d1", "track_id": "t1", "car_id": "c1", "timestamp_ns": 200, "tick": 2, "channels": {"Throttle": 0.5}},
    ]
    path.write_text("".join(json.dumps(r) + "\n" for r in rows), encoding="utf-8")
