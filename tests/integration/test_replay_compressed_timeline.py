...
def test_session_replay_endpoint_validates_monotonic_timeline(tmp_path: Path):
    ...
    frames = [
        {
            ...
            "timestamp_ns": 100,
            "tick": 1,
            "channels": {"Throttle": 0.5, "Brake": 0.0, "Speed": 10.0},
            "quality_flags": {},
        },
        {
            "session_id": "s1",
            ...
            "timestamp_ns": 200,
            "tick": 2,
            "channels": {"Throttle": 0.5, "Brake": 0.0, "Speed": 11.0},
            "quality_flags": {},
        },
    ]
    ...
    response = client.post(
        "/session/replay", json={"artifact_path": str(path), "sampling_hz": 60, "source": "jsonl"}
    )
